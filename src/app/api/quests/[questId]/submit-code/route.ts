...
import type { CodeSubmitResponse } from '@/lib/api/types';
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const response = await apiClient.post<CodeSubmitResponse>(
      `/quests/${questId}/submit-code`,
      { code: body.code },
      { token }
    );
    ...
    if (error instanceof Error && 'status' in error && 'detail' in error) {
      ...
      return NextResponse.json(
        { error: apiError.detail || 'Failed to submit code' },
